...
class CuentaRepository:
    ...
    async def obtener_dato_usuario_por_id(self, idUsuario: str, dato_requerido: str = "*"):
        try:
            ...
            if not response.data:
                logging.warning(f"No se encontró usuario con ID {idUsuario}")
                return None
            return response.data
        except APIError as e:
    ...
    async def obtener_id_y_nombre_usuario_por_correo_y_telefono(self, correo: str, telefono: str):
        try:
            ...
            if not response.data:
                logging.warning(f"No se encontró usuario con correo {correo} y teléfono {telefono}")
                return None
            return response.data
        except APIError as e:
    ...
    async def actualizar_usuario(self, idUsuario: str, datos_actualizados: dict):
        try:
            ...
            if not response.data:
                logging.warning(f"No se pudo actualizar usuario con ID {idUsuario}")
                return None
            return response.data
        except APIError as e:
    ...
    async def eliminar_usuario(self, idUsuario: str):
        try:
            ...
            if not response.data:
                logging.warning(f"No se pudo eliminar usuario con ID {idUsuario}")
                return None
            return response.data
        except APIError as e:
    ...
    async def obtener_administrador_por_id(self, idAdmin: str):
        try:
            ...
            if not response.data:
                logging.warning(f"No se encontró administrador con ID {idAdmin}")
                return None
            return response.data
        except APIError as e:
    ...
    async def obtener_id_nombre_y_rol_administrador_por_correo_matricula_y_contrasena(self, correo: str, matricula: str, contrasena: str):
        try:
            ...
            if not response.data:
                logging.warning(f"No se encontró administrador con correo {correo} y matrícula {matricula}")
                return None
            return response.data
        except APIError as e:
    ...
    async def eliminar_administrador(self, idAdmin: str):
        try:
            ...
            if not response.data:
                logging.warning(f"No se pudo eliminar administrador con ID {idAdmin}")
                return None
            return response.data
        except APIError as e:
